"""Llama Stack implementation for tools endpoint."""
...
async def tools_llama_stack(  # pylint: disable=too-many-locals,too-many-statements
    complete_mcp_headers: dict[str, dict[str, str]],
) -> ToolsResponse:
    """List all available tools from MCP servers using Llama Stack.

    Queries Llama Stack for all registered toolgroups (both MCP servers and
    built-in toolgroups like builtin::rag) and retrieves their tools.
    ...
    Raises:
        HTTPException: If unable to connect to Llama Stack or tool retrieval fails.
    """
    toolgroups_response = []
    try:
