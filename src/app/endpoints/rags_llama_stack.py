"""Llama Stack implementation for RAGs endpoints."""
...
async def list_rags_llama_stack() -> RAGListResponse:
    """
    List all available RAGs using Llama Stack.
...
async def get_rag_llama_stack(rag_id: str) -> RAGInfoResponse:
    """Retrieve a single RAG identified by its unique ID using Llama Stack.
    ...
    Raises:
        HTTPException: If unable to connect to Llama Stack or RAG not found.
    """
    llama_stack_configuration = configuration.llama_stack_configuration
    logger.info("Llama stack config: %s", llama_stack_configuration)
